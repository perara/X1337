...
bool Enemy::process()
{
...

	{
		...
		float x = sin(angle) * (Config::getInstance().gameClock.getElapsedTime().asSeconds() * 100);
		float y = cos(angle) * (Config::getInstance().gameClock.getElapsedTime().asSeconds() * 100);
